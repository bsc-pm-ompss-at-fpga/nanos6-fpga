...
AC_DEFUN([AC_CHECK_CACHE],
	[
		...
		if test x"${CACHELINE_WIDTH}" != x"" ; then
			...
			if test -f /sys/devices/system/cpu/cpu0/cache/index3/coherency_line_size ; then
			...
			if [ test -z "$cache_size" ] || [ test "$cache_size" -lt 1 ] ; then
				if test -f /sys/devices/system/cpu/cpu0/cache/index0/coherency_line_size ; then
