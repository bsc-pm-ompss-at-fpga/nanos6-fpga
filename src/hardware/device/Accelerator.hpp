...
public:
  ...
  Accelerator(int handler, nanos6_device_t type, uint32_t numOfStreams,
      ...
      : _stopService(false), _finishedService(false), _deviceHandler(handler),
        ...
        _streamPool(numOfStreams), _pollingPeriodUs(pollingPeriodUs),
...
public:
  ...
  int getDirectoryHandler() const { return _directoryHandler; }
  ...
  void setDirectoryHandle(int handle) { _directoryHandler = handle; }
  ...
  inline MemoryPlace *getMemoryPlace() { return _memoryPlace; }

  inline ComputePlace *getComputePlace() { return _computePlace; }

  inline nanos6_device_t getDeviceType() const { return _deviceType; }

  inline int getDeviceHandler() const { return _deviceHandler; }
  ...
  // to it.
  virtual void *getAsyncHandle() { return nullptr; };
  ...
  createEvent(std::function<void((AcceleratorEvent *))> onCompletion =
                  [](AcceleratorEvent *) {}) {
  ...
  virtual void destroyEvent(AcceleratorEvent *event) { delete event; }
